...
# 送月用粉红/暖色系
header_fill  = PatternFill('solid', fgColor='AD1457')   # 深玫红
summary_fill = PatternFill('solid', fgColor='FCE4EC')   # 浅粉
tier_fills = {
    '5000元以上':  PatternFill('solid', fgColor='F48FB1'),
    '2000-4999元': PatternFill('solid', fgColor='F8BBD9'),
    '1000-1999元': PatternFill('solid', fgColor='FDEBD0'),
    '600-999元':   PatternFill('solid', fgColor='E8F5E9'),
    '300-599元':   PatternFill('solid', fgColor='E3F2FD'),
    '200-299元':   PatternFill('solid', fgColor='EEEEEE'),
}
...
red_bold   = Font(name='微软雅黑', bold=True, color='AD1457', size=11)
...
ws1['A1'] = '宝宝送月红包明细'
ws1['A1'].font = Font(name='微软雅黑', bold=True, color='AD1457', size=16)
ws1['A1'].alignment = center
ws1.row_dimensions[1].height = 38

for col, h in enumerate(['序号', '姓名', '红包金额（元）', '金额档位'], 1):
...
ws2['A1'] = '宝宝送月红包 · 汇总统计'
ws2['A1'].font = Font(name='微软雅黑', bold=True, color='AD1457', size=15)
...
key_stats = [
    ...
    ('亲友人数', f'{count} 位'),
    ('人均红包', f'¥{avg:.0f}'),
    ('最高红包', f'{max_gift[0]}  ¥{max_gift[1]:,}'),
    ('最低红包', f'{min_gift[0]}  ¥{min_gift[1]:,}'),
...
for i, (label, val) in enumerate(key_stats):
    ...
    ws2.cell(row=row, column=2, value=val).font = Font(name='微软雅黑', bold=True, color='AD1457', size=11)
...
for col, h in enumerate(['金额档位', '人数', '占比', '红包小计', '占总额比'], 1):
    ws2.cell(row=15, column=col, value=h).font = white_bold
    ws2.cell(row=15, column=col).fill = PatternFill('solid', fgColor='C2185B')
...
for i, t in enumerate(order):
    ...
    for col, val in enumerate([t, tc, f'{tc/count*100:.1f}%', ts, f'{ts/total*100:.1f}%'], 1):
        ...
        if col == 4:
            c.number_format = '¥#,##0'
            c.font = Font(name='微软雅黑', bold=True, color='AD1457', size=10)
        else:
...
ws3['A1'] = '送月红包排行榜（从高到低）'
ws3['A1'].font = Font(name='微软雅黑', bold=True, color='AD1457', size=15)
ws3['A1'].alignment = center
ws3.row_dimensions[1].height = 38

for col, h in enumerate(['排名', '姓名', '红包金额（元）', '金额档位'], 1):
...
out = '/Users/xiaoxiami/.qclaw/workspace/宝宝送月红包表格.xlsx'
wb.save(out)
print(f"完成！红包总计: ¥{total:,}，共{count}人，人均¥{avg:.0f}")
